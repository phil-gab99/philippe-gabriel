...
describe(ProjectsComponent.name, () => {
  ...
  it('should render correct number of projects', () => {
    const projectCards: DebugElement[] = projectsFixture.debugElement.queryAll(
      By.css('[data-testid="project-cards"]')
    );
  ...
  it('should render projects card header', () => {
    const stackIcons: DebugElement[] = projectsFixture.debugElement.queryAll(
      By.css('[data-testid="stack-icon"]')
    );
    const names: DebugElement[] = projectsFixture.debugElement.queryAll(
      By.css('[data-testid="name"]')
    );
    const stackNames: DebugElement[] = projectsFixture.debugElement.queryAll(
      By.css('[data-testid="stack-name"]')
    );
    const dates: DebugElement[] = projectsFixture.debugElement.queryAll(
      By.css('[data-testid="date"]')
    );

    expect(
      stackIcons.map((e: DebugElement) =>
        e.nativeElement.getAttribute('fonticon')
      )
    ).toEqual(projectsDataMock.map((p: Project) => p.stack.icon));
    expect(
      names.map((e: DebugElement) => e.nativeElement.textContent.trim())
    ).toEqual(projectsDataMock.map((p: Project) => p.name));
    expect(
      stackNames.map((e: DebugElement) => e.nativeElement.textContent.trim())
    ).toEqual(projectsDataMock.map((p: Project) => p.stack.name));
    expect(
      dates.map((e: DebugElement) =>
        (e.nativeElement as HTMLSpanElement).textContent?.trim()
      )
    ).toEqual(
      projectsDataMock.map((p: Project) => `${p.dateStart} - ${p.dateEnd}`)
    );
  });

  it('should render projects card content', () => {
    const descriptions: DebugElement[] = projectsFixture.debugElement.queryAll(
      By.css('[data-testid="description"]')
    );

    expect(
      descriptions.map((e: DebugElement) => e.nativeElement.textContent.trim())
    ).toEqual(projectsDataMock.map((p: Project) => p.description));
  });

  it('should navigate to contact urls', () => {
    window.open = jest.fn();

    const sourceUrls: DebugElement[] = projectsFixture.debugElement.queryAll(
      By.css('[data-testid="source-url"]')
    );
