...
describe(NavbarComponent.name, () => {
  ...
  describe('Rendering', () => {
    ...
    beforeEach(() => {
      TestBed.overrideComponent(NavbarComponent, {
        add: {
          providers: [{ provide: BioService, useValue: bioServiceMock }],
    ...
    it('should render name', () => {
      const firstName: DebugElement = navbarFixture.debugElement.query(
        By.css('[data-testid="first-name"]')
      );
      const lastName: DebugElement = navbarFixture.debugElement.query(
        By.css('[data-testid="last-name"]')
      );

      expect(
        (firstName.nativeElement as HTMLSpanElement).textContent?.trim()
      ).toBe(bioDataMock.firstName);
      expect(
        (lastName.nativeElement as HTMLSpanElement).textContent?.trim()
      ).toBe(bioDataMock.lastName);
    });

    it('should render navigation items', () => {
      const navItems: DebugElement[] = navbarFixture.debugElement.queryAll(
        By.css('[data-testid="nav-items"]')
      );

      expect(navItems.length).toBe(navbarComponent.navItems.length);
      expect(
        navItems.map((e: DebugElement) =>
          (e.nativeElement as HTMLButtonElement).textContent?.trim()
        )
      ).toEqual(navbarComponent.navItems.map((n: NavItem) => n.title));
    });
  });
  ...
  describe('Navigation', () => {
    ...
    it('should navigate to home component', fakeAsync(() => {
      const homeButton: DebugElement = navbarHarness.routeDebugElement!.query(
    ...
    it('should navigate to navigation items', fakeAsync(() => {
      const navItems: DebugElement[] =
        navbarHarness.routeDebugElement!.queryAll(
          By.css('[data-testid="nav-items"]')
        );
      ...
      navItems.forEach((item: DebugElement, i: number) => {
        ...
        expect(navbarLocation.path()).toBe(
          `${navbarComponent.navItems[i].path}#${navbarComponent.navItems[i].fragment}`
        );
      });
    }));
  });
